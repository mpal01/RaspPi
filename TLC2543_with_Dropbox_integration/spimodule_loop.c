...
static PyObject* SPI_transfer(SPI *self, PyObject *args)
{
	...
	char* list;
	int length_list = 1;
	uint16_t delay = 15, fullVoltage=0;
	uint32_t speed = 1000;
	int i=0; //, oldByte=0, newByte=0;
...
/*This is the transfer part, and sets up
	...
	fullVoltage = rx[0];
	fullVoltage = (fullVoltage<<8)|rx[1];
	fullVoltage = (fullVoltage>>4);


	return Py_BuildValue("i,i,i,i,i,i", rx[0], rx[1], fullVoltage, rx[3], rx[4], rx[5]);
